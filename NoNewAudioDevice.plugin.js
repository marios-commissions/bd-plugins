/**
 * @name NoNewAudioDevice
 * @description Stops the new audio device modal from showing.
...
module.exports = (() => {
	...
	return !global.ZeresPluginLibrary ? class {
		...
		return class extends Plugin {
			start() {
				const handlers = Dispatcher._action;
				if (!handlers) return;

				const dependencies = handlers._dependencyGraph;
				if (!dependencies) return;

				const nodes = dependencies.nodes;
				if (!nodes) return;

				const store = Object.values(nodes);

				const ConnectedDeviceStore = store.find(e => e.name === 'ConnectedDeviceStore');
				if (!ConnectedDeviceStore) return;

				const actions = ConnectedDeviceStore.actionHandler;

				actions.CONNECTED_DEVICE_NEVER_SHOW_MODAL();
			}
		};
